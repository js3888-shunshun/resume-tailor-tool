...
- 2026-06-20 — feature: Application Q&A — ✅ Done. New "Application Q&A" tab: paste an application question (plus any requirements like a word limit) and optionally the JD, get an answer drawn from the browser library. `POST /answer` (`pipeline/answer_question` + `prompts/app_question`, reuses match's `_background`); first-person, truthful, respects length constraints, no em-dashes / AI cliches; returns plain text. UI has the question box, an optional JD box, a Generate button, an editable answer box with a live word count, and a one-click Copy button (clipboard API + execCommand fallback). pytest 96/96; live: a 120-word-limit behavioral question -> 119 words, STAR-structured, grounded in the real background, zero em-dashes.
- 2026-06-20 — features: section order + domain tags + JD match score — ✅ Done. (1) Section order: `projects_first` flag (RenderRequest/Generate/ResumeDocument) renders the projects/research section before professional experience; template refactored to a Jinja `entry_list` macro called in either order; UI "projects first" toggle in the Resume card and One-Click. (2) Domain tags: `Experience.domains`/`Project.domains` (free-form, e.g. finance/gaming), editable in the library editor (tag input) and shown as chips in the library view; carried in the browser-stored library so all steps see them. (3) JD match score: `POST /match` (`pipeline/match_score.score_match` + `prompts/match`) rates the library's fit to the JD 0-100 with a one-line summary + strengths/gaps; UI "Match score" button on the JD-analysis card with a clay score ring. pytest 93/93; live: a quant-dev JD vs an ML candidate scored 28/100 with accurate gaps (no C++/low-latency/finance).
